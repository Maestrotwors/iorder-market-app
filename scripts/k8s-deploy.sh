...
# 1. Ensure DB is running (in parallel with builds)
...
# 2. Build images (backend in parallel, frontend separately)
...
# 4. Rolling restart ALL at once (zero-downtime — 2+ replicas, maxUnavailable: 0)
